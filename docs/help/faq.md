...
If you are on an Arch based distro you can update the panel via:
...
## Why are my drop-down menus showing outside of the screen or at the incorrect position?

There can be various scaling factors applied to your display (gdk, hyprland, both) which can change how pixels are calculated on your screen by GDK applications (HyprPanel);

Since HyprPanel can't account for all different scaling scenarios you may have to adjust the `Scaling Priority` option in the settings dialog under `Configuration > General > Scaling` to ensure that the drop-down menus are spawned in the correct position.
